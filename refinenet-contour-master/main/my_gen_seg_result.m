...
function result_info=my_gen_seg_result(seg_param, ds_info, predict_info, epoch_no)
    ...
    %}    
    result_info.seg_eva_result_coarse = [];
    ...
    %if rem(epoch_no,2) == 0
    do_save_results(ds_info, seg_param, predict_mask, score_map_org, predict_result_densecrf, epoch_no);
    %end
end



function do_save_results(ds_info, seg_param, predict_mask_net, score_map_org, predict_result_densecrf, epoch_no)
    ...
    predict_mask_data=predict_mask_net;      
    ...
    if eva_param.save_predict_mask
        ...
        %tmp_dir = strcat(tmp_dir,'\','epoch_',int2str(epoch_no));
        mkdir_notexist(tmp_dir);
        one_cache_file=fullfile(tmp_dir, [img_name '.png']);
        soft_map = imresize(score_map_org,seg_param.img_size_input);
        soft_map = uint8(soft_map * 255);
        imwrite(soft_map, one_cache_file);
    end
